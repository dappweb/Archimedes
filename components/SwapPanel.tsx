...
const SwapPanel: React.FC = () => {
  ...
  const [isSelling, setIsSelling] = useState(false); // false = Buy JBC (Pay USDT), true = Sell JBC (Pay JBC)
  ...
  }, [payAmount, isSelling, poolUSDT, poolJBC]);
  ...
  const handleSwap = async () => {
      ...
      try {
          ...
          if (isSelling) {
              ...
              // Buy JBC: Approve USDT -> SwapMCToJBC
              if (mcContract) {
        ...
        <div className="space-y-3 md:space-y-4 relative z-10">
            ...
            <div className="bg-slate-100 p-3 md:p-4 rounded-lg md:rounded-xl border border-slate-200 transition-all focus-within:ring-2 focus-within:ring-macoin-500/50">
                <div className="flex justify-between text-xs md:text-sm text-slate-500 mb-2">
                    ...
                    <span className="truncate ml-2">{t.swap.balance}: {isSelling ? balanceJBC : balanceUSDT} {isSelling ? 'ARC' : 'USDT'}</span>
                </div>
            ...
            {/* Pool Stats */}
            <div className="mt-6 pt-4 border-t border-slate-100 grid grid-cols-2 gap-4 text-xs sm:text-sm">
                <div className="flex flex-col gap-1">
                    <span className="text-slate-400">Pool Liquidity (USDT)</span>
                    <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-macoin-500"></div> {parseFloat(poolUSDT).toLocaleString()} USDT</span>
                </div>
                <div className="flex flex-col gap-1">
                    <span className="text-slate-400">Pool Liquidity (ARC)</span>
                    <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-yellow-500"></div> {parseFloat(poolJBC).toLocaleString()} ARC</span>
                </div>
            </div>
